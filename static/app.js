...
const elements = {
    ...
    tabUniqualize: document.getElementById('tab-uniqualize'),
    ...
    btnTgStop: document.getElementById('btn-tg-stop'),
    ...
    // Uniqualize Elements
    uniqualizePanel: document.getElementById('uniqualize-panel'),
    uniqYandexFolder: document.getElementById('uniq-yandex-folder'),
    uniqVariantsCount: document.getElementById('uniq-variants-count'),
    btnStartUniqualize: document.getElementById('btn-start-uniqualize'),
    btnStopUniqualize: document.getElementById('btn-stop-uniqualize'),
    uniqSpinner: document.getElementById('uniq-spinner'),
    uniqSpinnerText: document.getElementById('uniq-spinner-text'),
    btnCopyUniqLinks: document.getElementById('btn-copy-uniq-links'),
    uniqConsoleBody: document.getElementById('uniq-console-body'),

    // Pack Generation
    btnGen5Packs: document.getElementById('btn-gen-5-packs'),
    btnGen10Packs: document.getElementById('btn-gen-10-packs'),
    packsStatusBar: document.getElementById('packs-status-bar'),
    packsStatusText: document.getElementById('packs-status-text'),
    packsProgressFill: document.getElementById('packs-progress-fill'),
    packsSlotFill: document.getElementById('packs-slot-fill'),
    packsProgressLabel: document.getElementById('packs-progress-label'),
    packsResultsSection: document.getElementById('packs-results-section'),
    packsResultsHeading: document.getElementById('packs-results-heading'),
    packsAccordion: document.getElementById('packs-accordion'),
    btnClearPacks: document.getElementById('btn-clear-packs'),
    btnCopyAllPackLinks: document.getElementById('btn-copy-all-pack-links')
};
...
function initializeApp() {
    ...
    // Check if pack generation is active in background on page load
    setTimeout(() => {
        pollPacksStatus(true);
    }, 1500);

    // Check if uniqualization is active on page load
    setTimeout(() => {
        pollUniqualizeStatus(true);
    }, 2000);
}
...
function setupEventListeners() {
    ...
    if (elements.tabUniqualize) {
        elements.tabUniqualize.addEventListener('click', () => switchTab('uniqualize'));
    }
    ...
    if (elements.btnTgStop) {
        elements.btnTgStop.addEventListener('click', stopTableGeneration);
    }
    ...
    // Pack Generation buttons
    if (elements.btnGen5Packs) {
        elements.btnGen5Packs.addEventListener('click', () => startPackGeneration(5));
    }
    if (elements.btnGen10Packs) {
        elements.btnGen10Packs.addEventListener('click', () => startPackGeneration(10));
    }
    if (elements.btnClearPacks) {
        elements.btnClearPacks.addEventListener('click', clearPacksResults);
    }
    if (elements.btnCopyAllPackLinks) {
        elements.btnCopyAllPackLinks.addEventListener('click', copyAllPackLinks);
    }

    // Uniqualize controls
    if (elements.btnStartUniqualize) {
        elements.btnStartUniqualize.addEventListener('click', startUniqualizationProcess);
    }
    if (elements.btnCopyUniqLinks) {
        elements.btnCopyUniqLinks.addEventListener('click', copyUniqualizedLinksToClipboard);
    }
}
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            if (bannerText) {
                ...
                // Centered Option 4A Layout (Pearl Volume Gradient)
                ...
                // Helper to split text logically into 2 lines
                const splitTextIntoTwoLines = (text) => {
                    ...
                    for (const keyword of [' — ', ' – ', ' - ', ' за ', ' всего ', ' от ']) {
                ...
                // 1. Fill background with Pearl Volume Gradient and apply soft shadow
                ctx.save();
                ctx.shadowColor = 'rgba(0, 0, 0, 0.3)';
                ctx.shadowBlur = Math.round(plaqueHeight * 0.16);
                ctx.shadowOffsetX = 3;
                ctx.shadowOffsetY = 5;
                
                const cardGradient = ctx.createLinearGradient(xPos, yPos, xPos + plaqueWidth, yPos);
                cardGradient.addColorStop(0, '#ffffff');
                cardGradient.addColorStop(1, '#ebf0f6');
                ctx.fillStyle = cardGradient;
                ctx.fill();
                ctx.restore();
                
                // 2. Draw left cyan accent tag
                ctx.save();
                ctx.beginPath();
                ctx.moveTo(xPos + radius, yPos);
                ctx.lineTo(xPos + plaqueWidth - radius, yPos);
                ...
                ctx.fillStyle = '#00f0ff'; // Bright cyan/greenish blue
                ctx.fillRect(xPos, yPos, 12, plaqueHeight);
                ctx.restore();
                
                // 3. Draw outer border (slate-300 / #cbd5e1)
                ctx.strokeStyle = '#cbd5e1';
                ctx.lineWidth = 2;
                ctx.stroke();
                
                // 4. Draw text (dark slate color, split into 2 lines, centered)
                ctx.fillStyle = '#0f172a';
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                
                const lines = splitTextIntoTwoLines(bannerText);
                const line1 = lines[0];
                const line2 = lines[1];
                
                // Horizontal center of text (+6px offset to balance left tag width)
                const textCenter = (width / 2) + 6;
                const maxTextWidth = plaqueWidth - Math.round(plaqueWidth * 0.08); // 8% padding
                
                if (line2) {
                    // Draw in 2 lines
                    let currentFontSize = Math.round(plaqueHeight * 0.28);
                    ctx.font = `bold ${currentFontSize}px "Plus Jakarta Sans", "Montserrat", "Arial", sans-serif`;
                    
                    // Prevent overflow of either line
                    while (
                        (ctx.measureText(line1).width > maxTextWidth || ctx.measureText(line2).width > maxTextWidth) && 
                        currentFontSize > Math.round(plaqueHeight * 0.16)
                    ) {
                        currentFontSize -= 1;
                        ctx.font = `bold ${currentFontSize}px "Plus Jakarta Sans", "Montserrat", "Arial", sans-serif`;
                    }
                    ...
                    const yCenter = yPos + (plaqueHeight / 2);
                    ctx.fillText(line1, textCenter, yCenter - Math.round(lineHeight * 0.5));
                    ctx.fillText(line2, textCenter, yCenter + Math.round(lineHeight * 0.5));
                } else {
                    // Draw in 1 line
                    let currentFontSize = Math.round(plaqueHeight * 0.32);
                    ctx.font = `bold ${currentFontSize}px "Plus Jakarta Sans", "Montserrat", "Arial", sans-serif`;
                    
                    while (ctx.measureText(line1).width > maxTextWidth && currentFontSize > Math.round(plaqueHeight * 0.18)) {
                        currentFontSize -= 1;
                        ctx.font = `bold ${currentFontSize}px "Plus Jakarta Sans", "Montserrat", "Arial", sans-serif`;
                    }
                    
                    ctx.fillText(line1, textCenter, yPos + (plaqueHeight / 2));
                }
            }
...
function switchTab(tabId) {
    if (tabId === 'creative') {
        ...
        if (elements.tabUniqualize) elements.tabUniqualize.classList.remove('active');
        
        elements.sidebarPanel.style.display = '';
        elements.workspacePanel.style.display = '';
        elements.tableGeneratorPanel.style.display = 'none';
        if (elements.uniqualizePanel) elements.uniqualizePanel.style.display = 'none';
    } else if (tabId === 'table') {
        ...
        pollTableGeneratorStatus(true);
    } else if (tabId === 'uniqualize') {
        elements.tabCreative.classList.remove('active');
        elements.tabTable.classList.remove('active');
        if (elements.tabUniqualize) elements.tabUniqualize.classList.add('active');
        
        elements.sidebarPanel.style.display = 'none';
        elements.workspacePanel.style.display = 'none';
        elements.tableGeneratorPanel.style.display = 'none';
        if (elements.uniqualizePanel) elements.uniqualizePanel.style.display = 'flex';
...
async function runTableGeneration() {
    ...
    if (elements.btnTgStop) {
        elements.btnTgStop.style.display = 'none'; // hide stop button during transitional request phase
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Toggle buttons
            elements.btnTgStart.style.display = 'none';
            if (elements.btnTgStop) {
                elements.btnTgStop.style.display = 'inline-flex';
                elements.btnTgStop.disabled = false;
            }
            ...
            elements.btnTgStart.style.display = 'inline-flex';
            if (elements.btnTgStop) elements.btnTgStop.style.display = 'none';
        }
    } catch (error) {
...
async function pollTableGeneratorStatus(quiet = false) {
    try {
        ...
        if (!status.active) {
            ...
            elements.btnTgStart.style.display = 'inline-flex';
            if (elements.btnTgStop) {
                elements.btnTgStop.style.display = 'none';
            }
            ...
            // Task is active, hide start button and show stop button
            elements.btnTgStart.style.display = 'none';
            if (elements.btnTgStop) {
                elements.btnTgStop.style.display = 'inline-flex';
                elements.btnTgStop.disabled = false;
...
async function stopTableGeneration() {
    elements.btnTgStop.disabled = true;
    elements.btnTgStop.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Останавливаем...';
    try {
        const response = await fetch('/api/table-generator/stop', {
            method: 'POST'
        });
        const result = await response.json();
        if (response.ok) {
            showNotification('Запуск остановки сборщика таблицы...', 'info');
        } else {
            showNotification(result.detail || 'Не удалось остановить сборщик', 'error');
            elements.btnTgStop.disabled = false;
            elements.btnTgStop.innerHTML = '<i class="fa-solid fa-stop"></i> Остановить сборку';
        }
    } catch (error) {
        console.error('Stop table generation error:', error);
        showNotification('Ошибка связи при остановке сборщика', 'error');
        elements.btnTgStop.disabled = false;
        elements.btnTgStop.innerHTML = '<i class="fa-solid fa-stop"></i> Остановить сборку';
    }
}
...
let packsPollingInterval = null;
let lastRenderedPackCount = 0;

async function startPackGeneration(count) {
    const adInput = elements.adInput ? elements.adInput.value.trim() : '';
    if (!adInput) {
        showNotification('Введите параметры объявления перед генерацией пачек', 'warning');
        return;
    }

    // Disable pack buttons during generation
    if (elements.btnGen5Packs) elements.btnGen5Packs.disabled = true;
    if (elements.btnGen10Packs) elements.btnGen10Packs.disabled = true;

    // Reset and show progress bar
    lastRenderedPackCount = 0;
    if (elements.packsStatusBar) elements.packsStatusBar.style.display = 'flex';
    if (elements.packsStatusText) elements.packsStatusText.textContent = `Запуск генерации ${count} пачек...`;
    if (elements.packsProgressFill) elements.packsProgressFill.style.width = '0%';
    if (elements.packsSlotFill) elements.packsSlotFill.style.width = '0%';
    if (elements.packsProgressLabel) elements.packsProgressLabel.textContent = `0 / ${count} пачек`;

    // Show accordion (empty with heading)
    if (elements.packsResultsSection) elements.packsResultsSection.style.display = 'flex';
    if (elements.packsResultsHeading) elements.packsResultsHeading.textContent = `Генерация ${count} пачек...`;
    if (elements.packsAccordion) elements.packsAccordion.innerHTML = '';

    try {
        const resp = await fetch('/api/generate-packs', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ad_input: adInput, count: count })
        });
        if (!resp.ok) {
            const err = await resp.json();
            throw new Error(err.detail || 'Ошибка запуска');
        }
        showNotification(`Запущена генерация ${count} пачек. Это займёт несколько минут.`, 'success');
        // Start polling
        startPacksPolling();
    } catch (err) {
        console.error('startPackGeneration error:', err);
        showNotification(`Ошибка запуска генерации пачек: ${err.message}`, 'error');
        if (elements.btnGen5Packs) elements.btnGen5Packs.disabled = false;
        if (elements.btnGen10Packs) elements.btnGen10Packs.disabled = false;
        if (elements.packsStatusBar) elements.packsStatusBar.style.display = 'none';
    }
}

function startPacksPolling() {
    if (packsPollingInterval) clearInterval(packsPollingInterval);
    packsPollingInterval = setInterval(() => pollPacksStatus(false), 2000);
}

function stopPacksPolling() {
    if (packsPollingInterval) {
        clearInterval(packsPollingInterval);
        packsPollingInterval = null;
    }
}

async function pollPacksStatus(initialCheck) {
    try {
        const resp = await fetch('/api/generate-packs/status');
        if (!resp.ok) return;
        const data = await resp.json();

        const isActive = data.active;
        const totalPacks = data.total_packs || 0;
        const currentPack = data.current_pack || 0;
        const currentSlot = data.current_slot || 0;
        const completedPacks = data.completed_packs || [];
        const message = data.message || '';

        if (isActive || completedPacks.length > 0) {
            // Show progress bar if active
            if (elements.packsStatusBar) {
                elements.packsStatusBar.style.display = isActive ? 'flex' : 'none';
            }

            if (isActive) {
                // Start polling if initial check found active state
                if (initialCheck) startPacksPolling();

                if (elements.packsStatusText) elements.packsStatusText.textContent = message;
                if (elements.packsProgressLabel) elements.packsProgressLabel.textContent = `${completedPacks.length} / ${totalPacks} пачек`;

                // Pack-level progress
                const packPct = totalPacks > 0 ? (completedPacks.length / totalPacks) * 100 : 0;
                if (elements.packsProgressFill) elements.packsProgressFill.style.width = packPct + '%';

                // Slot-level progress (within current pack)
                const slotPct = currentSlot > 0 ? (currentSlot / 9) * 100 : 0;
                if (elements.packsSlotFill) elements.packsSlotFill.style.width = slotPct + '%';

                // Show buttons are disabled
                if (elements.btnGen5Packs) elements.btnGen5Packs.disabled = true;
                if (elements.btnGen10Packs) elements.btnGen10Packs.disabled = true;
            } else {
                // Generation finished
                ...
                if (completedPacks.length > 0 && !initialCheck) {
                    showNotification(`Готово! Сгенерировано ${completedPacks.length} пачек.`, 'success');
                }
            }

            // Render any newly completed packs (incremental render)
            if (completedPacks.length > lastRenderedPackCount) {
                if (elements.packsResultsSection) elements.packsResultsSection.style.display = 'flex';
                if (elements.packsResultsHeading) {
                    elements.packsResultsHeading.textContent = isActive
                        ? `Пачки креативов (генерация...)`
                        : `Пачки креативов — готово ${completedPacks.length} шт.`;
                }
                // Render only new packs
                for (let i = lastRenderedPackCount; i < completedPacks.length; i++) {
                    renderPackItem(completedPacks[i]);
                }
                lastRenderedPackCount = completedPacks.length;
            }

        } else if (!initialCheck) {
            // No active session and no data — hide progress
            if (elements.packsStatusBar) elements.packsStatusBar.style.display = 'none';
        }
    } catch (err) {
        console.error('pollPacksStatus error:', err);
    }
}

function renderPackItem(pack) {
    if (!elements.packsAccordion) return;
    ...
    item.id = `pack-item-${packNum}`;

    // Build slots table rows
    const slotRows = slots.map(slot => {
        const imgCell = slot.yandex_url
            ? `<img src="${slot.yandex_url}" class="pack-slot-img" alt="Слот ${slot.slot_number}" onerror="this.style.display='none'">`
            : `<div class="pack-slot-img-placeholder"><i class="fa-solid fa-image-slash"></i></div>`;

        const urlCell = slot.yandex_url
            ? `<a href="${slot.yandex_url}" target="_blank">${slot.yandex_url.replace('https://', '')}</a>`
            : `<span class="no-url">—</span>`;

        return `
            <tr>
                <td class="pack-slot-num">#${slot.slot_number}</td>
                <td>${imgCell}</td>
                <td>
                    <div class="pack-slot-title">${slot.title || ''}</div>
                    <div class="pack-slot-banner">${slot.banner_text || ''}</div>
                </td>
                <td class="pack-slot-url">${urlCell}</td>
            </tr>`;
    }).join('');

    item.innerHTML = `
        <div class="pack-item-header" onclick="togglePackItem(${packNum})">
            <span class="pack-badge">Пачка ${packNum}</span>
            <span class="pack-item-name">${packName}</span>
            <span class="pack-item-status done"><i class="fa-solid fa-check-circle"></i> ${slots.length}/9 готово</span>
            <button class="pack-copy-btn" id="pack-copy-btn-${packNum}" onclick="event.stopPropagation(); copyPackLinks(${packNum})" title="Скопировать ссылки для вставки в Excel (одна ячейка)">
                <i class="fa-solid fa-copy"></i> Скопировать ссылки
            </button>
            <i class="fa-solid fa-chevron-right pack-chevron"></i>
        </div>
        <div class="pack-item-body">
            <table class="pack-slots-table">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Картинка</th>
                        <th>Текст</th>
                        <th>Ссылка (disk.yandex.ru)</th>
                    </tr>
                </thead>
    ...
    elements.packsAccordion.appendChild(item);

    // Auto-open latest pack
    togglePackItem(packNum);
    // Collapse previous packs
    if (packNum > 1) {
        const prevItem = document.getElementById(`pack-item-${packNum - 1}`);
        if (prevItem && prevItem.classList.contains('open')) {
            prevItem.classList.remove('open');
        }
    }
}

function togglePackItem(packNum) {
    const item = document.getElementById(`pack-item-${packNum}`);
    if (item) {
        item.classList.toggle('open');
    }
}

function copyPackLinks(packNum) {
    // Gather Yandex URLs from the rendered table rows for this pack
    const item = document.getElementById(`pack-item-${packNum}`);
    if (!item) return;

    const links = [];
    const rows = item.querySelectorAll('.pack-slot-url a');
    rows.forEach(a => {
        const url = a.href || a.textContent;
        if (url && url.includes('disk.yandex.ru')) {
            links.push(url);
        }
    });

    if (links.length === 0) {
        showNotification('Нет ссылок для копирования (картинки ещё не загружены?)', 'warning');
        return;
    }

    // Join with newline inside quotes so Excel treats all links as ONE cell with line breaks
    const text = '"' + links.join('\n').replace(/"/g, '""') + '"';

    navigator.clipboard.writeText(text).then(() => {
        showNotification(`Скопировано ${links.length} ссылок из пачки ${packNum} в буфер обмена для одной ячейки Excel!`, 'success');
        const btn = document.getElementById(`pack-copy-btn-${packNum}`);
        if (btn) {
        ...
        console.error('copyPackLinks error:', err);
        showNotification('Ошибка копирования ссылок', 'error');
    });
}

function copyAllPackLinks() {
    const packItems = document.querySelectorAll('.pack-item');
    if (packItems.length === 0) {
        showNotification('Нет пачек для копирования', 'warning');
        return;
    }

    const allPacksTextLines = [];
    let totalLinks = 0;
    ...
    packItems.forEach(item => {
        ...
        if (links.length > 0) {
            // Wrap in quotes with newlines — Excel keeps all links in ONE cell with line breaks
            allPacksTextLines.push('"' + links.join('\n').replace(/"/g, '""') + '"');
            totalLinks += links.length;
        }
    });

    if (allPacksTextLines.length === 0) {
        showNotification('Нет ссылок для копирования (картинки ещё не загружены?)', 'warning');
        return;
    }

    // Join packs with CRLF so each pack lands in a separate row in Excel
    const text = allPacksTextLines.join('\r\n');

    navigator.clipboard.writeText(text).then(() => {
        showNotification(`Успешно скопировано ${totalLinks} ссылок (${allPacksTextLines.length} пачек)! Вставьте в Excel — каждая пачка займет ровно одну ячейку.`, 'success');
        const btn = elements.btnCopyAllPackLinks;
        if (btn) {
            const orig = btn.innerHTML;
            btn.innerHTML = '<i class="fa-solid fa-check"></i> Скопировано!';
            btn.style.background = 'var(--accent-green)';
        ...
        console.error('copyAllPackLinks error:', err);
        showNotification('Ошибка копирования всех ссылок', 'error');
    });
}

function clearPacksResults() {
    stopPacksPolling();
    lastRenderedPackCount = 0;
    if (elements.packsAccordion) elements.packsAccordion.innerHTML = '';
    if (elements.packsResultsSection) elements.packsResultsSection.style.display = 'none';
    if (elements.packsStatusBar) elements.packsStatusBar.style.display = 'none';
    if (elements.btnGen5Packs) elements.btnGen5Packs.disabled = false;
    if (elements.btnGen10Packs) elements.btnGen10Packs.disabled = false;
}
...
function clearUniqConsole() {
    if (elements.uniqConsoleBody) {
        elements.uniqConsoleBody.innerHTML = '<div class="console-line system" style="color: var(--text-muted);">[Система] Консоль очищена.</div>';
    }
}

// Append log line to Uniqualizer console
function appendUniqConsoleLog(message, type = 'system') {
    if (!elements.uniqConsoleBody) return;
    const lineEl = document.createElement('div');
    lineEl.className = `console-line ${type}`;
    ...
    lineEl.innerText = message;
    elements.uniqConsoleBody.appendChild(lineEl);
    elements.uniqConsoleBody.scrollTop = elements.uniqConsoleBody.scrollHeight;
}

// Start polling status
async function pollUniqualizeStatus(quiet = false) {
    try {
        const response = await fetch('/api/uniqualize/status');
        if (!response.ok) return;
        
        const status = await response.json();
        
        // Output progress message if changed
        if (status.message && status.message !== lastUniqMessage) {
            let logType = 'system';
            if (status.message.toLowerCase().includes('ошибка') || status.message.toLowerCase().includes('fail')) {
                logType = 'error';
            } else if (status.message.toLowerCase().includes('готово') || status.message.toLowerCase().includes('завершено')) {
                logType = 'success';
            }
            appendUniqConsoleLog(status.message, logType);
            lastUniqMessage = status.message;
        }
        ...
        if (status.active) {
            ...
            if (elements.btnStartUniqualize) {
                elements.btnStartUniqualize.disabled = true;
                elements.btnStartUniqualize.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Обработка...';
            }
            if (elements.uniqSpinner) {
                elements.uniqSpinner.style.display = 'flex';
                const percent = status.total_variants > 0 ? Math.round((status.done_variants / status.total_variants) * 100) : 0;
                elements.uniqSpinnerText.innerText = `Выполнено: ${status.done_variants}/${status.total_variants} (${percent}%)`;
            }
            if (elements.btnCopyUniqLinks) elements.btnCopyUniqLinks.style.display = 'none';

            // Start polling if not already
            if (!uniqPollInterval) {
                uniqPollInterval = setInterval(() => pollUniqualizeStatus(false), 2000);
            }
        } else {
            // Inactive
            if (uniqPollInterval) {
                clearInterval(uniqPollInterval);
                uniqPollInterval = null;
            }
            
            if (elements.btnStartUniqualize) {
                elements.btnStartUniqualize.disabled = false;
                elements.btnStartUniqualize.innerHTML = '<i class="fa-solid fa-play"></i> Запустить уникализацию';
            }
            if (elements.uniqSpinner) elements.uniqSpinner.style.display = 'none';

            if (uniqualizedResultLinks.length > 0) {
                if (elements.btnCopyUniqLinks) elements.btnCopyUniqLinks.style.display = 'block';
                if (status.message.toLowerCase().includes('готово') && !quiet) {
                    showNotification('Уникализация успешно завершена!', 'success');
                }
            }
            
            if (status.error) {
                if (!quiet) showNotification('Ошибка уникализации', 'error');
                appendUniqConsoleLog(`[Критическая ошибка] ${status.error}`, 'error');
            }
        }
    } catch (error) {
        console.error('Error polling uniqualization status:', error);
    }
}

// Start processing
async function startUniqualizationProcess() {
    const yandexFolder = elements.uniqYandexFolder.value.trim();
    const count = parseInt(elements.uniqVariantsCount.value) || 10;
    
    if (!yandexFolder) {
        showNotification('Укажите папку на Яндекс.Диске с исходными фото', 'warning');
        return;
    }
    
    try {
        clearUniqConsole();
        appendUniqConsoleLog(`Запуск процесса уникализации для папки «${yandexFolder}»...`);
        appendUniqConsoleLog(`Количество вариантов на фото: ${count}`);
        ...
        if (response.ok && result.status === 'success') {
            showNotification('Процесс уникализации запущен в фоновом режиме.', 'success');
            pollUniqualizeStatus(false);
        } else {
            showNotification(result.detail || 'Не удалось запустить уникализацию', 'error');
            appendUniqConsoleLog(`[Ошибка запуска] ${result.detail || 'Неизвестная ошибка'}`, 'error');
        }
    } catch (error) {
        console.error('Start uniqualize error:', error);
        showNotification('Ошибка сети при запуске уникализации', 'error');
    }
}
