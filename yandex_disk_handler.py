...
class YandexDiskHandler:
    ...
    def upload_file(self, local_file_path: str, disk_file_path: str, overwrite: bool = True) -> Optional[str]:
        ...
        try:
            ...
            if meta_response.status_code == 200:
                ...
                if public_url:
                    return self.ensure_disk_url(public_url)
                else:
    ...
    def publish_and_get_link(self, path: str) -> Optional[str]:
        """Publishes a resource at path and returns its public download/view link in disk.yandex.ru format."""
        encoded_path = urllib.parse.quote(path)
        try:
            # 1. Publish
            ...
            if response.status_code == 200:
                ...
                return self.ensure_disk_url(url) if url else None
            return None
        except Exception as e:
            print(f"Error publishing resource: {e}")
            return None

    def ensure_disk_url(self, url: str) -> str:
        """Converts any Yandex Disk public URL to disk.yandex.ru/i/ format (required by Avito)."""
        if not url:
            return url
        # yadi.sk/i/XXX -> disk.yandex.ru/i/XXX
        if "yadi.sk/i/" in url:
            key = url.split("yadi.sk/i/")[-1].split("?")[0]
            return f"https://disk.yandex.ru/i/{key}"
        # yadi.sk/d/XXX -> disk.yandex.ru/d/XXX
        if "yadi.sk/d/" in url:
            key = url.split("yadi.sk/d/")[-1].split("?")[0]
            return f"https://disk.yandex.ru/d/{key}"
